...
void RF24::getFlag(void) {
  printf("%s", ofir ? "true" : "false");
}
...
bool RF24::begin(void)
{
  ...
  //---------------------------------------------------------------------
  //---------------------------------------------------------------------
  uint8_t SYMAX_bind_rx_tx_addr[] = {0xab, 0xac, 0xad, 0xae, 0xaf};
...
float *judy = (float*)&buf;
  ...
  while(!ofir) {
	...
	//Start Writing
	//startFastWrite(buf,len,multicast); --------OFIR-------
...
void RF24::SYMAX_send_packet(uint8_t bind)
{
...
void RF24::build_packet_x5c(uint8_t bind)
{
  if (bind) {
...
uint8_t RF24::checksum(uint8_t *data)
{
  uint8_t sum = data[0];
...
void RF24::SYMAX_read_controls(uint8_t* throttle, uint8_t* rudder, uint8_t* elevator, uint8_t* aileron, uint8_t* flags)
{
...
uint8_t RF24::convert_channel(uint8_t num) {
